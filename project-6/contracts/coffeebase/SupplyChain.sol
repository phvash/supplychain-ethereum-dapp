...
contract SupplyChain is RetailerRole, ConsumerRole{
  ...
  // pssoible improvement:
  // would be useful to have a state to represent 'default' different from harvested
  // so it would be possible to differentiate items in the default state and the harvested state
  // currently if an item that doesn't exist is fetched, zero is still returned for the state (since the type is uint)
  // which creates a bit of confusion
  ...
  // Define a modifier that checks the price and refunds the remaining balance to distributor
